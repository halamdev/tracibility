...
export const WalletConnection: React.FC<WalletConnectionProps> = ({
    ...
    return (
      <div className="text-center">
        <button
          onClick={onConnect}
          disabled={loading}
          className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <Wallet className="w-5 h-5" />
          <span>{loading ? 'Đang kết nối...' : 'Kết nối ví'}</span>
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center space-x-4">
      <div className="flex items-center space-x-2 bg-green-50 text-green-800 px-4 py-2 rounded-lg border border-green-200">
